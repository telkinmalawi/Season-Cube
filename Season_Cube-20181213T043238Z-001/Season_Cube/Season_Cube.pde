...
Flower[] flowerList = new Flower[8];
int array_size = 8;

PShape Seeds;
Seed[] seedList = new Seed[108];
void setup() {
  ...
  for (int l = 0; l < 6; l++){
  for (int h = 0; h < 18; h++) {
    seedList[h*6+l] = new Seed(200, 450 + rows, -200, 1);
  }
  rows = (l*50);
  }
  ...
  for (int i=0; i < array_size; i++) {
    PImage addThis = loadImage("flower"+ i +".png");
    addThis.resize(100,100); 
    Flowers.add(addThis);
     for (int j=0; j < flowerList.length; j++) {
    flowerList[j] = new Flower("flower"+ j +".png",mouseX,mouseY);
  }
  }
}
void draw() {
  ...
  //danelion
   for (int i = 0; i < Flowers.size(); i ++ ) {
    PImage temp = Flowers.get(i);
    image(temp, 10+i*20, 500+i*Yposi);
  }
  for (int j=0; j < flowerList.length; j++) {
  flowerList[j].display();
  }
}
...
void keyPressed() {
  ...
  int j = int(random(8));
  Flower oneMore = new Flower("flower"+ j +".png",mouseX,mouseY);
  flowerList = (Flower[]) append(flowerList, oneMore);
}
void leaves() {
...
void rain() {
  translate(0, 0);
  if( roty>= -1.4345992 && roty<=1.5453987 && rotx>=-1.2846006 && rotx<=0.7053982||roty>= -1.4345992 && roty<=1.5453987 && rotx<=6.2654247 && rotx>=4.8753986){
...
void seeds(){
if(rotx>=-0.54460126 && rotx<=-0.2646015 && roty>=1.225398 && roty<=1.5853976){
  for (int h = 0; h < 108; h ++ ) { 
    pushMatrix();
    rotateY((radians(-4))*(h));
    rotateX((radians(-0.1))*h);
    seedList[h].display();
    seedList[h].grow();
    seedList[h].move();
    seedList[h].repeat();
